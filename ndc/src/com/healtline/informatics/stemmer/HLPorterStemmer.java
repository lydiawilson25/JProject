package com.healtline.informatics.stemmer;

/**
 *
 * Extension of the Porter Stemmer to deal with some issues around medically
 * related terms.  This resulted in the addition of some exception such as
 * - News -- news
 * - Organization --> organize
 * - Ageing --> aging
 * and the removal of some "porter" rules such as
 * - logi --> log
 * - itis --> "nothing"
 *
 * The core of the code is based on the Porter stemmer present in Lucene.
 *
 * author: Igor Perisic
 *
 *
 * Lucene porter copyright here below:
 *
...
/*
 ...
 - Rules changes:
 itis -> itis i.e. keep it as is.
 logy --> logi (not to nothing)
 ...
 S rule:
 news
 blues keep as is (fixing bug 1340 around baby blues matching blue baby)
 testes -> testi (testicle goes to testicl)
...
public class HLPorterStemmer {
	...
	private boolean HLexception = false;
	...
	private final boolean equals(String s) {
		int l = s.length();
		if (l != (k + 1)) {
			return false;
		}
	 ...
	 * ponies -> poni ties -> ti caress -> caress cats -> cat
	 * 
	...
	private final void step1() {
		if (b[k] == 's') {
			if (ends("sses")) {
				...
				HLexception = true;
				return;
			} else if (equals("blues")) {
				HLexception = true;
				return;
			} else if (equals("testes")) {
				HLexception = true;
				b[k - 1] = 'i';
				k--;
			} else if (ends("itis")) {
		...
		if (ends("eed")) {
			...
			/*
			 * checking for the ageing and aging
			 */
			if (equals("aging")) {
				HLexception = true;
				return;
			} else if (equals("ageing")) {
				HLexception = true;
				for (i = 2; i < 6; i++) {
					b[i] = b[i + 1];
				}
	...
	private final void step3() {
		...
		case 'o':
			...
			if (ends("ation")) {
				if (equals("publication") || equals("medication")
						|| equals("ulceration")) {
					HLexception = true;
				} else {
					r("ate");
		...
		case 't':
			...
			break;
		/*
		 * keeping the -logy as -logi and not matching it to the -logic case
		 * 'g': if (ends("logi")) { r("log"); break; }
		 * 
		 */
		}
	}
	...
	private final void step4() {
		...
		case 'l':
			if (ends("ical")) {
				if (equals("clinical") || equals("topical")
						|| equals("technical")) {
					HLexception = true;
				} else {
					r("ic");
	...
	private final void step5() {
		...
		case 'i':
			if (ends("ic")) {
				if (equals("organic") || equals("periodic")
						| equals("systemic") || equals("alcoholic")
						|| equals("colonic") || equals("enteric")
						|| equals("acetic") || equals("diabetic")
						|| equals("ironic")) {
					j += 2;
					HLexception = true;
				}
		...
		case 's':
			if (ends("ism")) {
				if (equals("alcoholism")) {
					r("ic");
					j += 2;
					HLexception = true;
				} else if (equals("organism")) {
					HLexception = true;
					j += 3;
				}
				break;
		...
		case 'z':
			if (ends("ize")) {
				/*
				 * finishing the truncation for organization and organize
				 */
				if (equals("organize") || equals("immunize")) {
					j += 3;
					HLexception = true;
				}
				break;
	...
	private final void step6() {
		j = k;
		if (b[k] == 'e') {
			if (equals("age") || equals("ice") | equals("haemorrhage")
					|| equals("hemorrhage") || equals("device")) {
				HLexception = true;
				return;
			}
			int a = m();
	...
	public static void main(String[] args) {
		HLPorterStemmer s = new HLPorterStemmer();
